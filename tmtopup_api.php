...
if($_SERVER['REMOTE_ADDR'] == '203.146.127.115' && isset($_GET['request']))
{
	...
	if($_GET['request'] != false)
	{
		...
		$result = mysql_query('SELECT `id` FROM `'. $_CONFIG['mysql']['tbname'] .'` WHERE `'. $_CONFIG['mysql']['field_username'] .'`=\'' . mysql_real_escape_string($request['Ref1']) . '\' LIMIT 1') or die(mysql_error());
		if(mysql_num_rows($result) == 1)
		{
			$row = mysql_fetch_assoc($result);
			if(mysql_query('UPDATE `'. $_CONFIG['mysql']['tbname'] .'` SET `' . $_CONFIG['TMN']['point_field_name'] . '`=`' . $_CONFIG['TMN']['point_field_name'] . '`+' . $_CONFIG['TMN'][$request['cardcard_amount']]['point'] . ' WHERE `'. $_CONFIG['mysql']['field_username'] .'`=' . $row[$_CONFIG['mysql']['field_username']]) == false)
			{
